...
extraMetricsList = ['records', 'uniqDD', 'uniqFLN', 'sumSC', 
				'maxSC', 'sdSC', 'skewSC', 'kurtSC', 
				'maxDDSC', 'sdDDSC', 'skewDDSC', 'kurtDDSC',
				'maxFLNSC', 'sdFLNSC', 'skewFLNSC', 'kurtFLNSC',
				'DDMaxSC1', 'DDMaxSC2']
...
# to remove FLN keys from vnDict
...
def outputSvmFormatFile(fReader,LAST_ROW,outF,for_test,idxDic):
	...
	for row in fReader:
		...
		if((preVN != curVN and len(vnFeatDic)>0) or count == LAST_ROW):
			...
			## ************* CUSTOM FEATURES END  *************  ##
			# Remove FLN 5k feats
			entries_to_remove(list(range(flnStart,nextStart)), vnFeatDic)
