...
ddFlnOd=collections.OrderedDict()
odDic = {'wOD':wOD,'upcOD':upcOD,'ddOD':ddOD,'flnOD':flnOD,'ddFlnOd':ddFlnOd}
...
def initDictionaries(fReader,LAST_ROW,idxDic,odDic):
	...
	for row in fReader:
		...
		if fln.isdigit():
			if fln not in flnOD:
				flnOD[fln] = 1
			ddFlnKey = str(dd) + '_' + str(fln)
			if ddFlnKey not in ddFlnOd:
				ddFlnOd[ddFlnKey] = 1
#TRAIN
...
extraMetricsList = ['records', 'uniqDD', 'uniqFLN', 'uniqDDFLN', 'sumSC', 
...
ddFlnStart = ddStart + len(ddList)
flnStart = ddFlnStart + len(ddFlnList)
...
def outputSvmFormatFile(fReader,LAST_ROW,outF,for_test,idxDic):
	...
	for row in fReader:
		...
		if((preVN != curVN and len(vnFeatDic)>0) or count == LAST_ROW):
			...
			vnFeatDic[emStart + extraMetricsList.index('uniqDDFLN')] = len(np.unique(ddFlnVnList))
			...
			ddVnScList = [vnFeatDic.get(x) for x in list(range(ddStart,ddFlnStart)) if vnFeatDic.get(x)!=None]
			if len(ddVnScList)>1:
			...
			flnVnScList = [vnFeatDic.get(x) for x in list(range(ddFlnStart,nextStart)) if vnFeatDic.get(x)!=None]
			...
			ddVnFeatDicSub = {k: vnFeatDic.get(k) for k in list(range(ddStart,ddFlnStart)) if vnFeatDic.get(k)!=None}
			if len(ddVnFeatDicSub) > 1: #TODO: check sc also, if that is greater than ONE than add DDMaxSC1 atleast
...
# 				vnFeatDic[flnIdx] = vnFeatDic[flnIdx] + sc
			ddFlnKey = str(dd) + '_' + str(fln)
			ddFlnVnList.append(ddFlnKey)
			ddFlnIdx = ddFlnStart + ddFlnList.index(ddFlnKey)
			if ddFlnIdx not in vnFeatDic:
				vnFeatDic[ddFlnIdx] = sc
			else:
				vnFeatDic[ddFlnIdx] = vnFeatDic[ddFlnIdx] + sc
...
for ddFln in ddFlnList:
	dictWriter.writerow([ddFlnStart+ddFlnList.index(ddFln), ddFln])
for fln in flnList:
...
dictWriter.writerow(['ddFlnStart', ddFlnStart])
