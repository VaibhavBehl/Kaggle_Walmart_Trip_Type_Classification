...
upcStart = ddStart + len(ddList)
flnStart = upcStart + len(upcList)
...
def outputSvmFormatFile(fReader,LAST_ROW,outF,for_test,idxDic):
	...
	for row in fReader:
		...
		if((preVN != curVN and len(vnFeatDic)>0) or count == LAST_ROW):
			...
			ddVnScList = [vnFeatDic.get(x) for x in list(range(ddStart,upcStart)) if vnFeatDic.get(x)!=None]
			if len(ddVnScList)>1:
			...
			flnVnScList = [vnFeatDic.get(x) for x in list(range(upcStart,flnStart)) if vnFeatDic.get(x)!=None]
			...
			ddVnFeatDicSub = {k: vnFeatDic.get(k) for k in list(range(ddStart,upcStart)) if vnFeatDic.get(k)!=None}
